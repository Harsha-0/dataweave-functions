...
var dateFields = ["observationBeginDateTime","observationEndDateTime","dateTimeOfTransaction","specimenReceivedDateTime","resultsDateTime"]
...
// Main result map where the OBR object is generated by applying reMap to each element of mapping and payload
var result = {
  OBR: keysOf(mapping) map ({($): reMap(mapping[($)], updatedPayload[($)]) }) reduce ((item, acc = {}) -> acc ++ item)
}

// Build the restructure map by processing the result.OBR and filtering based on maxFieldValue

var restructure = ((0 to maxFieldValue) map (inputData(result.OBR) filterObject ((value, key, index) -> key ~= $)))map (($$ + 1): valuesOf($)[0] default "") reduce ((item, acc = {}) -> acc ++ item)
