...
var dateFields = ["dateOfBirth","clientLastUpdatedDate"]
...
var result = {
  QPD: keysOf(mapping) map ({($): reMap(mapping[($)], updatedPayload[($)]) }) reduce ((item, acc = {}) -> acc ++ item)
}

// Build the restructure map by processing the result.QPD and filtering based on maxFieldValue

var restructure = ((0 to maxFieldValue) map (inputData(result.QPD) filterObject ((value, key, index) -> key ~= $)))map (($$ + 1): valuesOf($)[0] default "") reduce ((item, acc = {}) -> acc ++ item)
