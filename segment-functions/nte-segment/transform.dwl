...
// Main result map where the NTE object is generated by applying reMap to each element of mapping and payload
var result = {
  NTE: keysOf(mapping) 
...
// Build the restructure map by processing the result.NTE and filtering based on maxFieldValue

var restructure = ((0 to maxFieldValue) map (inputData(result.NTE) filterObject ((value, key, index) -> key ~= $)))map (($$ + 1): valuesOf($)[0] default "") reduce ((item, acc = {}) -> acc ++ item)
