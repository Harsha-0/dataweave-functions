...
var dateFields = ["dateTimeOfTransaction","orderEffectiveDateTime","fillersExpectedAvailabilityDateTime"]
...
// Main result map where the ORC object is generated by applying reMap to each element of mapping and payload
var result = {
  ORC: keysOf(mapping) map ({($): reMap(mapping[($)], updatedPayload[($)]) }) reduce ((item, acc = {}) -> acc ++ item)
}

// Build the restructure map by processing the result.ORC and filtering based on maxFieldValue

var restructure = ((0 to maxFieldValue) map (inputData(result.ORC) filterObject ((value, key, index) -> key ~= $)))map (($$ + 1): valuesOf($)[0] default "") reduce ((item, acc = {}) -> acc ++ item)
