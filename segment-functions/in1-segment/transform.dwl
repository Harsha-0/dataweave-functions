...
var dateFields = ["planEffectiveDate","planEffectiveDate","insuredDateOfBirth","noticeOfAdmissionDate",
"reportOfEligibilityDate","verificationDateTime","signatureCodeDate"]
...
// Main result map where the IN1 object is generated by applying reMap to each element of mapping and payload
var result = {
  IN1: keysOf(mapping) map ({($): reMap(mapping[($)], updatedPayload[($)]) }) reduce ((item, acc = {}) -> acc ++ item)
}

// Build the restructure map by processing the result.IN1 and filtering based on maxFieldValue

var restructure = ((0 to maxFieldValue) map (inputData(result.IN1) filterObject ((value, key, index) -> key ~= $)))map (($$ + 1): valuesOf($)[0] default "") reduce ((item, acc = {}) -> acc ++ item)
