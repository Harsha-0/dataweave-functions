...
// Main result map where the RXR object is generated by applying reMap to each element of mapping and payload
var result = {
  RXR: keysOf(mapping) 
...
// Build the restructure map by processing the result.RXR and filtering based on maxFieldValue

var restructure = ((0 to maxFieldValue) map (inputData(result.RXR) filterObject ((value, key, index) -> key ~= $)))map (($$ + 1): valuesOf($)[0] default "") reduce ((item, acc = {}) -> acc ++ item)
