...
// Main result map where the PV1 object is generated by applying reMap to each element of mapping and payload
var result = {
  PV1: keysOf(mapping) 
...
// Build the restructure map by processing the result.PV1 and filtering based on maxFieldValue

var restructure = ((0 to maxFieldValue) map (inputData(result.PV1) filterObject ((value, key, index) -> key ~= $)))map (($$ + 1): valuesOf($)[0] default "") reduce ((item, acc = {}) -> acc ++ item)
