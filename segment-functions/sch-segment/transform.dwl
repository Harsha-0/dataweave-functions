...
// Function to detect if a string value looks like a date
fun isDate(value: Any): Boolean =
  (value is String) and (try(() -> value as LocalDateTime {format: "yyyy-MM-dd'T'HH:mm:ss"}).success or try(() -> value as Date {format: "yyyy-MM-dd"}).success)

// Recursive function to traverse the object and parse dates dynamically
fun processDates(obj) =
  obj mapObject ((value, key) -> 
    if (isDate(value)) (key): parseDate(value)
    else if (value is Object) (key): processDates(value)
    else (key): value
  )

// Process the payload
var updatedPayload = processDates(payload)

// Main result map where the SCH object is generated by applying reMap to each element of mapping and payload
var result = {
  SCH: keysOf(mapping) map ({($): reMap(mapping[($)], updatedPayload[($)]) }) reduce ((item, acc = {}) -> acc ++ item)
}

// Build the restructure map by processing the result.SCH and filtering based on maxFieldValue

var restructure = ((0 to maxFieldValue) map (inputData(result.SCH) filterObject ((value, key, index) -> key ~= $)))map (($$ + 1): valuesOf($)[0] default "") reduce ((item, acc = {}) -> acc ++ item)
